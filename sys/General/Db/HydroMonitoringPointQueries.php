...
class HydroMonitoringPointQueries extends BaseQueries {
	...
	public static function getById($id, string $primaryKey = 'id'): ?array {
		...
		if ($hydroPoint) {
			...
			$hydroPoint['observedProperties'] = (new Select())
			...
			$hydroPoint['showObservedProperty'] = (new Select())
	 ...
	 * Once done, passes id and observed property data to {@see HydroMonitoringPointQueries::saveMeteopointObservedProperty()}.
	 *
	 * @param array  $data
	 * @param null   $id
	 ...
	 *
	 * @throws QueryException
	 * @uses \Environet\Sys\General\Db\HydroMonitoringPointQueries::prepareData()
	 * @uses \Environet\Sys\General\Db\HydroMonitoringPointQueries::saveHydropointObservedProperty()
		...
		if (!empty($values)) {
			...
			foreach ($values as $key => $id) {
				...
				if (empty($id) && in_array((int) $key, $assignedProperties)) {
					(new Delete())
						->table('hydropoint_observed_property')
						->where('mpointid = :mpointId')
